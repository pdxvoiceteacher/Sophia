...
def test_rebraid_audit_missing_input_fail_closed() -> None:
    ...
    assert payload['findings'][0]['type'] == 'rebraid-alert'


def test_rebraid_audit_alert_true_warn(tmp_path: Path) -> None:
    bridge = tmp_path / 'bridge'
    _write(bridge / 'rebraid_signal_map.json', {'generatedAt': '2027-01-01T00:00:00Z', 'rebraidAlert': True, 'rebraidStrength': 0.8})
...
def test_rebraid_audit_alert_false_info(tmp_path: Path) -> None:
    bridge = tmp_path / 'bridge'
    _write(bridge / 'rebraid_signal_map.json', {'generatedAt': '2027-01-01T00:00:00Z', 'rebraidAlert': False})
    payload = module.build_outputs(bridge_root=bridge)
...
def test_compat_entrypoint_writes_file(tmp_path: Path) -> None:
    bridge = tmp_path / 'bridge'
    _write(bridge / 'rebraid_signal_map.json', {'generatedAt': '2027-01-01T00:00:00Z', 'rebraidAlert': True})

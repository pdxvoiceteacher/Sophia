...
def test_dedupe_max_proposals_cap_when_both_triggers(tmp_path: Path):
    run_dir = tmp_path / "run"
    run_dir.mkdir()

    # Psi low AND UCC error present => historically could yield two cap actions for the same json_set.
    telemetry = {
    ...
    # Count how many max_proposals_per_run caps exist
    ...
    caps = [a for a in actions if a.get("op") == "json_set" and a.get("json_path") == "max_proposals_per_run" and a.get("value") == 1]
    assert len(caps) == 1
